...
plot_time_series_for_country <- function(country.confirmed, country.deaths, country.recovered, country.name, public.spaces.closed.at=NA, confinement.at=NA) {
  ...
  plot(country.confirmed, main= paste("Cases in", country.name), type = "o", xlab = "Days Since January 21, 2020", ylab = "Number of Cases", col = "red", xlim =  c(day.start, day.count + 4), ylim = ylim)
...
WARNING: The figures do not seem to be updated since March 23, 2020.
...
for (i in seq(1, country.count)) {
  d <- nrow(countries.recovered[[i]])
  y <- countries.confirmed[[i]][1:d] - (countries.recovered[[i]][1:d] + countries.deaths[[i]][1:d])
