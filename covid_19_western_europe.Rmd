...
plot_time_series_for_country <- function(country.confirmed, country.deaths, country.recovered, country.population=NA, country.name="", public.spaces.closed.at=NA, confinement.at=NA, trend.angle=20) {
  ...
  plot(country.confirmed, main= paste("Cases in", country.name), type = "o", xlab = "Days Since January 21, 2020", ylab = "Number of Cases", col = "red", xlim =  c(day.start, day.count + 15), ylim = ylim)
  ...
  last.progression.pct <- round((100 * dy) / last.confirmed[1])
  text(x = day.count - (6 + trend.angle / 20), y = (0.4 - (trend.angle/3000)) * ylim[2], paste(dy, "cases/w.\nAvg:", dy %/% 7, "case/day\n", last.progression.pct, "% progress last week"), srt = trend.angle, col = "black")
  ...
  if (!is.na(public.spaces.closed.at)) {
    ...
    text(x = public.spaces.closed.at - 15, y = ypos, "All public spaces are closed", col = "blue", srt = 90)
  }
  if (!is.na(confinement.at)) {
    abline(v = confinement.at, col = "red", lw = 1, lty = 5)
    text(x = confinement.at - 1, y = ypos, "Population is confined", col = "red", srt = 90)
  }
  ...
  if(!is.na(country.population)) {
    proportion <- country.confirmed/(country.population/100000)
    plot(seq(2, day.count), diff(proportion), type = "h", col = "blue", xlim =  c(day.start + 1, day.count), main = paste("New Confirmed Cases per Day per 100'000 Persons", country.name, sep = " in "), xlab = "Days Since January 21, 2020", ylab = "Number of Cases per Day per 100K P.", lwd = 5)
    grid()
  }
  ...
  if (!is.na(country.recovered)) {
    ...
    if(!is.na(country.population)) {
      proportion <- diffs/(country.population/100000)
      plot(seq(2, day.count), proportion, type = "h", col = "red", xlim =  c(day.start + 1, day.count), main = paste("New Active Cases per Day per 100'000 Persons", country.name, sep = " in "), xlab = "Days Since January 21, 2020", ylab = "Number of New Active Cases per 100K P.", lwd=5)
      grid()
    }
  }
}
...
countries.population <- list(switzerland.population, italy.population, france.population, germany.population, austria.population, spain.population, uk.population, denmark.population, norway.population, sweden.confirmed, iceland.population)
...
## Switzerland
```{r, echo=FALSE, warning=FALSE}
plot_time_series_for_country(switzerland.confirmed, switzerland.deaths, switzerland.recovered, switzerland.population, "Switzerland", 55, NA, trend.angle = 70)
```

## Italy
```{r, echo=FALSE, warning=FALSE}
plot_time_series_for_country(italy.confirmed, italy.deaths, italy.recovered, italy.population, "Italy", NA, trend.angle = 65)
```

## France
```{r, echo=FALSE, warning=FALSE}
plot_time_series_for_country(france.confirmed, germany.deaths, france.recovered, france.population, "France", 53, 56, trend.angle = 65)
```

## Germany
```{r, echo=FALSE, warning=FALSE}
plot_time_series_for_country(germany.confirmed, switzerland.deaths, germany.recovered, germany.population, "Germany", 48, NA, trend.angle = 45)
```

## Austria
```{r, echo=FALSE, warning=FALSE}
plot_time_series_for_country(austria.confirmed, austria.deaths, austria.recovered, austria.population, "Austria", 52, NA, trend.angle = 60)
```
...
## Denmark
```{r, echo=FALSE, warning=FALSE}
plot_time_series_for_country(denmark.confirmed, denmark.deaths, denmark.recovered, denmark.population, "Denmark", NA, NA, trend.angle = 50)
```
...
## Sweden
```{r, echo=FALSE, warning=FALSE}
plot_time_series_for_country(sweden.confirmed, sweden.deaths, sweden.recovered, sweden.population, "Sweden", NA, NA, trend.angle = 30)
```

## Iceland
```{r, echo=FALSE, warning=FALSE}
plot_time_series_for_country(iceland.confirmed, iceland.deaths, iceland.recovered, iceland.population, "Iceland", NA, NA, trend.angle = 0)
```
...
plot_time_series_for_country(usa.confirmed, usa.deaths, NA, NA, "USA", NA, NA, trend.angle=25)
```
...
plot(xrange, yrange/800, main= "New Confirmed Cases per Day per 100K Persons (last 10 days)", type = "n", xlab = "Days Since 2020-01-21", ylab = "Number of New Cases per Day per 100K P.")
grid()
legend("topleft", legend = country.names, col = col, lty = 1, lw = 4, title = "Country", ncol = 4, cex = 0.9)
for (i in seq(1, country.count)) {
  lines(seq(2, day.count) - 0.45 + 0.08 * i, diff(countries.confirmed[[i]])/(countries.population[[i]][1]/100000), type = "h", col = col[i], lty = 1, lw = 4)
}
```
